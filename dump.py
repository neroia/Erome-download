...
def _get_final_download_path(album_title: str) -> Path:
    """Create an album directory inside the user's Videos folder."""
    configured_root = os.environ.get("EROME_DOWNLOAD_ROOT")
    if configured_root:
        videos_path = Path(configured_root)
    else:
        home = Path.home()
        videos_path = home / "Videos"
        localized_videos_path = home / "Vídeos"
        if not videos_path.exists() and localized_videos_path.exists():
            videos_path = localized_videos_path

    final_path = videos_path / DOWNLOAD_DIR / album_title
    ...
    async with semaphore:
        ...
        try:
            ...
            if progress_callback:
                progress_callback(album_progress.tasks[album_task].completed, stats["total"])
